...
class ContextualUnet(nn.Module):
    def __init__(self, in_channels, n_feat=256, n_classes=10):
        ...
        self.out = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(n_feat, self.in_channels, kernel_size=3, stride=1, padding=1)
        )
    ...
    def forward(self, x, context, time, context_mask):
        ...
        cemb1 = self.context_embed1(context).view(-1, self.n_feat * 2, 1)
        cemb2 = self.context_embed2(context).view(-1, self.n_feat, 1)
        temb1 = self.time_embed1(time).view(-1, self.n_feat * 2, 1)
        temb2 = self.time_embed2(time).view(-1, self.n_feat, 1)
